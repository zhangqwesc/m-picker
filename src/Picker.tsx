...
class Picker extends React.Component<IPickerProp & IPickerProps, any> {
  ...
  scrollHanders = (() => {
    ...
    return {
      ...
      mouseup: (evt: React.MouseEvent<HTMLDivElement>) => this.onClick(evt.target as HTMLDivElement),
      getValue: () => {
        return scrollY
      },
      scrollTo,
  ...
  fireItemClick = (selectedValue) => {
    if (selectedValue !== this.state.selectedValue) {
      return
    }
    if (this.props.onItemClick) {
      this.props.onItemClick(selectedValue)
    }
  }
  ...
  onClick = (target: HTMLDivElement) => {
    this.fireItemClick(target.getAttribute('data-value'))
  }
